...
import {
  ...
  TEXT_PARTIAL_FONT_SELECTION,
  TEXT_PARTIAL_FONT_SINGLE_LINE_ADD_OPTIONS,
  TEXT_PARTIAL_FONT_TARGET_STYLE,
  TEXT_PARTIAL_FONT_TEXT,
  TEXT_PARTIAL_FONT_TWO_LINE_ADD_OPTIONS
...
test.describe('Редактирование текста и история объекта', () => {
  test.describe('изменение текста и история', () => {
    ...
    test('заблокированный текст не меняет содержимое и стиль через updateText', async({
      editorModel,
      history,
      text
    }) => {
      await test.step('Заблокировать текущий текстовый объект', async() => {
        await editorModel.lockSelectedObject()
        await history.flushPendingSave()
      })

      const beforeTextObject = await test.step('Получить состояние текста до попытки обновления', async() => {
        return text.getObject({ objectIndex: 0 })
      })

      const updateResult = await test.step('Попробовать изменить текст и стиль заблокированного объекта', async() => {
        return text.updateStyle({
          objectIndex: 0,
          style: {
            text: 'Новый заблокированный текст',
            color: '#ff0000',
            italic: true
          }
        })
      ...
      await test.step('Проверить что текстовый объект не изменился', async() => {
        const currentTextObject = await text.getObject({ objectIndex: 0 })
        const activeObject = await editorModel.getActiveObject()

        expect(updateResult).toBeNull()
        expect(currentTextObject?.locked).toBe(true)
        expect(currentTextObject?.text).toBe(beforeTextObject?.text)
        expect(currentTextObject?.fill).toBe(beforeTextObject?.fill)
        expect(currentTextObject?.fontStyle).toBe(beforeTextObject?.fontStyle)
        expect(activeObject?.id).toBe(currentTextObject?.id)
      })
    })
  })
  ...
  test.describe('Шрифт и размер после частичного выделения', () => {
    test('после смены шрифта и размера у части однострочного текста весь объект получает новые значения', async({
      text
    }) => {
      await test.step('Добавить однострочный текст с базовым шрифтом и размером', async() => {
        const textObject = await text.add(TEXT_PARTIAL_FONT_SINGLE_LINE_ADD_OPTIONS)

        text.checkCreation({ textObject })
      })

      await test.step('Выделить часть строки и поменять шрифт с размером', async() => {
        await text.enterTextEditing({ objectIndex: 0 })
        await text.setTextSelection({
          objectIndex: 0,
          ...TEXT_PARTIAL_FONT_SELECTION
        ...
        await text.updateStyle({
          objectIndex: 0,
          style: TEXT_PARTIAL_FONT_TARGET_STYLE
        })
      })

      await test.step('Выбрать текст как обычный объект и проверить его базовый стиль', async() => {
        await text.exitTextEditing({ objectIndex: 0 })
        await text.select({ objectIndex: 0 })

        const textObject = await text.getObject({ objectIndex: 0 })

        expect(textObject?.isEditing).toBe(false)
        expect(textObject?.fontFamily).toBe(TEXT_PARTIAL_FONT_TARGET_STYLE.fontFamily)
        expect(textObject?.fontSize).toBe(TEXT_PARTIAL_FONT_TARGET_STYLE.fontSize)
      })
    })

    test('частичная смена шрифта и размера в многострочном тексте не становится стилем всего объекта', async({
      text
    }) => {
      await test.step('Добавить многострочный текст с базовым шрифтом и размером', async() => {
        const textObject = await text.add(TEXT_PARTIAL_FONT_TWO_LINE_ADD_OPTIONS)

        text.checkCreation({ textObject })
      })

      await test.step('Выделить часть первой строки и поменять шрифт с размером', async() => {
        await text.enterTextEditing({ objectIndex: 0 })
        await text.setTextSelection({
          objectIndex: 0,
          ...TEXT_PARTIAL_FONT_SELECTION
      ...
      await test.step('Проверить что новый шрифт применился только к затронутой строке', async() => {
        const firstLineStyle = await text.getSelectionStyles({
          objectIndex: 0,
          start: 0,
          end: 1
        })
        const secondLineStart = TEXT_PARTIAL_FONT_TEXT.length + 1
        ...
        expect(firstLineStyle?.fontFamily).toBe(TEXT_PARTIAL_FONT_TARGET_STYLE.fontFamily)
        expect(firstLineStyle?.fontSize).toBe(TEXT_PARTIAL_FONT_TARGET_STYLE.fontSize)
        expect(secondLineStyle?.fontFamily).toBe(TEXT_PARTIAL_FONT_BASE_STYLE.fontFamily)
        expect(secondLineStyle?.fontSize).toBe(TEXT_PARTIAL_FONT_BASE_STYLE.fontSize)
      })

      await test.step('Выбрать текст как обычный объект и проверить что базовый стиль не изменился', async() => {
        await text.exitTextEditing({ objectIndex: 0 })
        await text.select({ objectIndex: 0 })
        ...
        expect(textObject?.isEditing).toBe(false)
        expect(textObject?.fontFamily).toBe(TEXT_PARTIAL_FONT_BASE_STYLE.fontFamily)
        expect(textObject?.fontSize).toBe(TEXT_PARTIAL_FONT_BASE_STYLE.fontSize)
      })
    })
  })
