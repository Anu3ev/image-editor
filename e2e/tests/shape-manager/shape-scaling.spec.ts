...
test.describe('Скейлинг шейпа с текстом по диагонали', () => {
  test.describe('Пропорциональный скейлинг по диагонали', () => {
    test('без Shift квадратный шейп остаётся пропорциональным', async({
      shapes
    }) => {
      ...
      const liveSnapshot = await test.step('Скейлить шейп по диагонали пропорционально без Shift', async() => {
    ...
    test('при сужении за правый нижний угол текст переносится во время drag и размер не дёргается после mouseup', async({
      shapes
    }) => {
      ...
      const liveSnapshot = await test.step('Сузить шейп пропорционально за правый нижний угол во время drag', async() => {
        return shapes.scaleDiagonallyProportionally({
          scale: 0.55,
          corner: 'br',
          objectIndex: 0
    ...
    test('при сужении за правый верхний угол текст переносится во время drag и размер не дёргается после mouseup', async({
      shapes
    }) => {
      await test.step('Добавить квадратный шейп с текстом из нескольких слов', async() => {
        await shapes.addShapeWithText({
      ...
      const liveSnapshot = await test.step('Сузить шейп пропорционально за правый верхний угол во время drag', async() => {
        return shapes.scaleDiagonallyProportionally({
          scale: 0.55,
          corner: 'tr',
          objectIndex: 0
  ...
  test.describe('Непропорциональный скейлинг по диагонали', () => {
    test('с зажатым Shift квадратный шейп может стать непропорциональным', async({
      shapes
    }) => {
      ...
      const liveSnapshot = await test.step('Скейлить шейп непропорционально по диагонали с Shift', async() => {
    ...
    test('текст в одну строку сразу переносится и увеличивает высоту шейпа', async({
      shapes
    }) => {
      ...
      const liveSnapshot = await test.step('Сжать шейп непропорционально по диагонали за верхний правый угол', async() => {
        return shapes.scaleDiagonally({
          scaleX: 0.45,
          scaleY: 0.6,
          corner: 'tr',
      ...
      // eslint-disable-next-line max-len
      const liveText = await test.step('Получить состояние текста во время уменьшения ширины путём непропорционального скейлинга по диагонали', async() => {
        return shapes.getTextNode({ objectIndex: 0 })
      })
    ...
    // eslint-disable-next-line max-len
    test('если двухстрочный текст уже упёрся в минимальную высоту, при уменьшении ширины текст начинает переноситься', async({
      shapes
    }) => {
      ...
      const liveText = await test.step('Получить состояние текста во время непропорционального скейлинга по диагонали', async() => {
        return shapes.getTextNode({ objectIndex: 0 })
      })
...
test.describe('Непропорциональный скейлинг пустого шейпа по диагонали', () => {
  test.beforeEach(async({ shapes }) => {
    await shapes.addEmptyTextShape()
  })
...
test.describe('Масштабирование скругления', () => {
  test('после равномерного увеличения сохраняет значение скругления', async({ shapes }) => {
    const initialSnapshot = await test.step('Добавить прямоугольник со скруглением 50 и получить исходный размер', async() => {
    ...
    await test.step('Проверить что размер вырос, а значение скругления осталось прежним', async() => {
      const shape = await shapes.getFirstShape()
      const finalSnapshot = await shapes.getScaleSnapshot({ objectIndex: 0 })

      expect(shape.shapeRounding).toBe(50)
      expect(finalSnapshot.groupBoundsWidth)
        .toBeGreaterThan(initialSnapshot.groupBoundsWidth + SHAPE_SCALING_TOLERANCE.direction)
      expect(finalSnapshot.groupBoundsHeight)
        .toBeGreaterThan(initialSnapshot.groupBoundsHeight + SHAPE_SCALING_TOLERANCE.direction)
    })
  })

  test('после уменьшения сохраняет значение скругления', async({ shapes }) => {
    const initialSnapshot = await test.step('Добавить прямоугольник со скруглением 80 и получить исходный размер', async() => {
      await shapes.add({ presetKey: 'square' })
    ...
    await test.step('Проверить что размер уменьшился, а значение скругления осталось прежним', async() => {
      const shape = await shapes.getFirstShape()
      const finalSnapshot = await shapes.getScaleSnapshot({ objectIndex: 0 })

      expect(shape.shapeRounding).toBe(80)
      expect(finalSnapshot.groupBoundsWidth)
        .toBeLessThan(initialSnapshot.groupBoundsWidth - SHAPE_SCALING_TOLERANCE.direction)
      expect(finalSnapshot.groupBoundsHeight)
        .toBeLessThan(initialSnapshot.groupBoundsHeight - SHAPE_SCALING_TOLERANCE.direction)
  ...
  test('после непропорционального растяжения сохраняет значение скругления', async({ shapes }) => {
    const initialSnapshot = await test.step('Добавить прямоугольник со скруглением 50 и получить исходный размер', async() => {
      await shapes.add({ presetKey: 'square' })
      await shapes.setRounding({ rounding: 50, objectIndex: 0 })
    ...
    await test.step('Проверить что шейп стал заметно шире своей высоты, а значение скругления осталось прежним', async() => {
      const shape = await shapes.getFirstShape()
      const finalSnapshot = await shapes.getScaleSnapshot({ objectIndex: 0 })
      ...
      expect(finalSnapshot.groupBoundsHeight)
        .toBeGreaterThan(initialSnapshot.groupBoundsHeight + SHAPE_SCALING_TOLERANCE.direction)
      expect(finalSnapshot.groupBoundsWidth)
        .toBeGreaterThan(finalSnapshot.groupBoundsHeight + SHAPE_SCALING_TOLERANCE.direction)
    })
  })
})
