...
import {
  ...
  SHAPE_PADDING_TEXT_EDITING_VERTICAL_OPTIONS,
  SHAPE_PADDING_TEXT_EDITING_VERTICAL_UPDATED_TEXT,
  SHAPE_PADDING_TOO_LARGE_RIGHT,
  SHAPE_PADDING_TOO_LARGE_TOP,
  SHAPE_PADDING_TOP_THEN_RIGHT_OPTIONS,
...
test.describe('Внутренние отступы текста внутри фигуры', () => {
  ...
  test.describe('изменение уже созданной фигуры', () => {
    ...
    test('слишком большой правый отступ уменьшается до максимально возможного значения', async({ shapes }) => {
      ...
      await test.step('Проверить что отступ зажался и текст остался внутри фигуры', () => {
        expect(updatedShape, 'обновление правого отступа должно вернуть фигуру').not.toBeNull()

        if (!updatedShape) {
          throw new Error('фигура должна существовать после изменения правого отступа')
        }
    ...
    test('после смены пресета доступные отступы сохраняются, а текст остаётся внутри фигуры', async({ shapes }) => {
      ...
      await test.step('Проверить что доступные отступы сохранились, а текст остался внутри фигуры', () => {
        expect(updatedShape, 'смена пресета должна вернуть фигуру').not.toBeNull()

        if (!updatedShape) {
          throw new Error('фигура должна существовать после смены пресета')
        }
  ...
  test('при изменении правого отступа сохраняет верхний отступ и не переносит текст по буквам', async({
    shapes
  }) => {
    const { id: shapeId } = SHAPE_PADDING_TOP_THEN_RIGHT_OPTIONS

    await test.step('Добавить фигуру с коротким текстом', async() => {
      await shapes.add({
        presetKey: 'square',
        options: SHAPE_PADDING_TOP_THEN_RIGHT_OPTIONS
      })
    })
    ...
    const shapeWithTopPadding = await test.step('Задать заведомо слишком большой верхний отступ', async() => {
      return shapes.update({
        id: shapeId,
        options: {
          textPadding: {
            top: SHAPE_PADDING_TOO_LARGE_TOP
          }
        }
      })
    })
    const textAfterTopPadding = await test.step('Получить состояние текста после верхнего отступа', () => {
      return shapes.getTextNode({ id: shapeId })
    })

    const shapeWithRightPadding = await test.step('Задать заведомо слишком большой правый отступ', async() => {
      return shapes.update({
        id: shapeId,
        options: {
          textPadding: {
    ...
    const finalSnapshot = await test.step('Получить итоговую геометрию фигуры и текста', () => {
      return shapes.getScaleSnapshot({ id: shapeId })
    })
    ...
    await test.step('Проверить что правый отступ ограничился без схлопывания верхнего', () => {
      expect(shapeWithTopPadding, 'обновление верхнего отступа должно вернуть фигуру').not.toBeNull()
      expect(shapeWithRightPadding, 'обновление правого отступа должно вернуть фигуру').not.toBeNull()
      expect(textAfterTopPadding, 'текст после верхнего отступа должен существовать').not.toBeNull()
      expect(finalText, 'итоговый текст должен существовать').not.toBeNull()

      if (
        !shapeWithTopPadding
        || !shapeWithRightPadding
        || !textAfterTopPadding
        || !finalText
      ) {
        throw new Error('фигура и текст должны существовать после изменения отступов')
      }

      const widthChange = Math.abs(finalSnapshot.groupBoundsWidth - initialSnapshot.groupBoundsWidth)
      const heightChange = Math.abs(finalSnapshot.groupBoundsHeight - initialSnapshot.groupBoundsHeight)

      expect(textAfterTopPadding.lineCount).toBe(1)
      expect(shapeWithRightPadding.shapePaddingTop).toBe(shapeWithTopPadding.shapePaddingTop)
      expect(shapeWithRightPadding.shapePaddingRight).toBeLessThan(SHAPE_PADDING_TOO_LARGE_RIGHT)
      expect(finalText.lineCount).toBe(textAfterTopPadding.lineCount)
      expect(widthChange).toBeLessThanOrEqual(1.5)
      expect(heightChange).toBeLessThanOrEqual(1.5)
  ...
  test('при переносе строки внутри фигуры верхний и нижний отступ не схлопываются, а высота растёт', async({ shapes }) => {
    const { id: shapeId } = SHAPE_PADDING_TEXT_EDITING_VERTICAL_OPTIONS

    await test.step('Добавить фигуру с большими верхним и нижним отступами', async() => {
      await shapes.add({
        presetKey: 'square',
        options: SHAPE_PADDING_TEXT_EDITING_VERTICAL_OPTIONS
      })
    })
    ...
    await test.step('Открыть редактирование и добавить вторую строку', async() => {
      await shapes.enterTextEditing({ id: shapeId })
      await shapes.updateEditingText({
        id: shapeId,
        text: SHAPE_PADDING_TEXT_EDITING_VERTICAL_UPDATED_TEXT
      })
    })

    const updatedShape = await test.step('Получить состояние фигуры после переноса строки', () => {
      return shapes.getObject({ id: shapeId })
    })
    const updatedSnapshot = await test.step('Получить размеры фигуры после переноса строки', () => {
      return shapes.getScaleSnapshot({ id: shapeId })
    })
    const updatedText = await test.step('Получить состояние текста после переноса строки', () => {
      return shapes.getTextNode({ id: shapeId })
    })

    await test.step('Проверить что фигура стала выше, а верхний и нижний отступ сохранились', () => {
      expect(initialShape, 'исходная фигура должна существовать').not.toBeNull()
      expect(initialText, 'исходный текст должен существовать').not.toBeNull()
      expect(updatedShape, 'фигура после переноса строки должна существовать').not.toBeNull()
      expect(updatedText, 'текст после переноса строки должен существовать').not.toBeNull()

      if (!initialShape || !initialText || !updatedShape || !updatedText) {
        throw new Error('фигура и текст должны существовать до и после переноса строки')
      }

      expect(updatedText.lineCount).toBe(initialText.lineCount + 1)
      expect(updatedShape.shapePaddingTop).toBe(initialShape.shapePaddingTop)
      expect(updatedShape.shapePaddingBottom).toBe(initialShape.shapePaddingBottom)
      expect(updatedSnapshot.groupBoundsHeight).toBeGreaterThan(initialSnapshot.groupBoundsHeight + 1.5)
      expect(Math.abs(updatedSnapshot.groupBoundsWidth - initialSnapshot.groupBoundsWidth)).toBeLessThanOrEqual(1.5)
  ...
  test(
    'после переноса строки при vertical scaling верхний и нижний отступ снова схлопываются без рывка после отпускания мыши',
    async({ shapes }) => {
      const { id: shapeId } = SHAPE_PADDING_TEXT_EDITING_VERTICAL_OPTIONS
      ...
      await test.step('Открыть редактирование, добавить вторую строку и выйти из него', async() => {
        await shapes.enterTextEditing({ id: shapeId })
        await shapes.updateEditingText({
          id: shapeId,
          text: SHAPE_PADDING_TEXT_EDITING_VERTICAL_UPDATED_TEXT
        })
        await shapes.exitTextEditing({ id: shapeId })
      })

      const expandedShape = await test.step('Получить состояние фигуры после переноса строки', () => {
        return shapes.getObject({ id: shapeId })
      })
      const expandedSnapshot = await test.step('Получить размеры фигуры после переноса строки', () => {
        return shapes.getScaleSnapshot({ id: shapeId })
      })
      const expandedText = await test.step('Получить состояние текста после переноса строки', () => {
        return shapes.getTextNode({ id: shapeId })
      })

      const liveSnapshot = await test.step('Сжать фигуру по высоте до упора', async() => {
        return shapes.shrinkToMinimumHeight({ id: shapeId })
      })
      ...
      const finalShape = await test.step('Получить итоговые отступы фигуры', () => {
        return shapes.getObject({ id: shapeId })
      })
      const finalText = await test.step('Получить итоговое состояние текста', () => {
        return shapes.getTextNode({ id: shapeId })
      ...
      await test.step('Проверить что vertical scaling снова может съедать верхний и нижний отступ', () => {
        expect(expandedShape, 'фигура после переноса строки должна существовать').not.toBeNull()
        expect(expandedText, 'текст после переноса строки должен существовать').not.toBeNull()
        expect(finalShape, 'фигура после vertical scaling должна существовать').not.toBeNull()
        expect(finalText, 'текст после vertical scaling должен существовать').not.toBeNull()

        if (!expandedShape || !expandedText || !finalShape || !finalText) {
          throw new Error('фигура и текст должны существовать до и после vertical scaling')
        }

        const expandedVerticalPadding = expandedShape.shapePaddingTop + expandedShape.shapePaddingBottom
        const finalVerticalPadding = finalShape.shapePaddingTop + finalShape.shapePaddingBottom
        ...
        expect(expandedText.lineCount).toBeGreaterThan(1)
        expect(liveSnapshot.groupBoundsHeight).toBeLessThan(expandedSnapshot.groupBoundsHeight - 1.5)
        expect(finalVerticalPadding).toBeLessThan(expandedVerticalPadding)
        expect(finalText.lineCount).toBe(expandedText.lineCount)
