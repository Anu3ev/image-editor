...
test.describe('Размер текста внутри фигуры', () => {
  ...
  test('при увеличении размера текста фигура ведёт себя одинаково до открытия и после открытия режима редактирования текста', async({ shapes }) => {
    ...
    await test.step('Проверить что размеры фигуры и переносы строк совпадают', async() => {
      ...
      expect(Math.abs((editingSnapshot.textBoundsWidth ?? 0) - (regularSnapshot.textBoundsWidth ?? 0))).toBeLessThanOrEqual(1.5)
      expect(Math.abs((editingSnapshot.textBoundsHeight ?? 0) - (regularSnapshot.textBoundsHeight ?? 0))).toBeLessThanOrEqual(1.5)
  ...
  test('если одна буква в строке перестаёт помещаться, фигура становится шире, чтобы текст не выходил за её пределы', async({ shapes }) => {
    await test.step('Открыть режим редактирования текста и сначала довести текст до переноса по строкам', async() => {
      await shapes.enterTextEditing({ objectIndex: 0 })
      await shapes.updateTextStyleInEditing({
        objectIndex: 0,
        style: {
    ...
    const wrappedSnapshot = await test.step('Получить состояние после переноса по строкам', () => {
      return shapes.getScaleSnapshot({ objectIndex: 0 })
    })
    ...
    await test.step('Проверить что фигура расширилась, а текст остался внутри', async() => {
      ...
      expect(expandedSnapshot.groupBoundsWidth).toBeGreaterThan(wrappedSnapshot.groupBoundsWidth + 1)
