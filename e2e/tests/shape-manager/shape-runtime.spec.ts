...
test.describe('Текст внутри уже добавленной фигуры после внешнего обновления', () => {
  // eslint-disable-next-line max-len
  test('после внешней замены текста длинный текст внутри фигуры сохраняет стиль каждой строки', async({ editorModel, shapes, template }) => {
    await test.step('Применить стандартный шаблон с текстом внутри фигуры', async() => {
      const insertedCount = await template.applyTemplate({
        template: SHAPE_TEMPLATE_WITH_STANDARD_TEXT_IN_FIGURE
      })
    ...
    await test.step('Внешне обновить текст уже добавленной фигуры', async() => {
    ...
    await test.step('Открыть обновлённый текст внутри фигуры', async() => {
      const textNode = await shapes.enterTextEditing({ objectIndex: 0 })
  ...
  // eslint-disable-next-line max-len
  test('после внешнего сокращения текста и продолжения ввода новая строка не получает стиль удалённой строки', async({ shapes, template }) => {
    await test.step('Применить стандартный шаблон с текстом внутри фигуры', async() => {
      const insertedCount = await template.applyTemplate({
        template: SHAPE_TEMPLATE_WITH_STANDARD_TEXT_IN_FIGURE
      })
    ...
    await test.step('Внешне сократить текст фигуры до одной строки', async() => {
      const updatedShape = await shapes.update({
        objectIndex: 0,
        options: {
          text: '69'
    ...
    await test.step('Открыть редактирование и добавить новую строку', async() => {
      const textNode = await shapes.enterTextEditing({ objectIndex: 0 })

      expect(textNode?.text).toBe('69')

      const updatedTextNode = await shapes.updateEditingText({
        objectIndex: 0,
        text: '69\nНОВОЕ'
      })

      expect(updatedTextNode?.text).toBe('69\nНОВОЕ')
      expect(updatedTextNode?.lineCount).toBe(2)
    })

    await test.step('Проверить что новая строка получила стиль оставшейся строки, а не удалённой', async() => {
      const firstCharacterStyle = await shapes.getSelectionStyles({
        objectIndex: 0,
        start: 3,
        end: 4
