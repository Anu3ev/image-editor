...
test.describe('Добавление фигуры с сохранением пропорций', () => {
  ...
  test('после добавления стрелки с сохранением пропорций и роста под текст следующая смена фигуры использует исходный replace box', async({ shapes }) => {
    const createdShape = await test.step('Добавить стрелку с сохранением пропорций и крупным текстом', async() => {
      return shapes.add({
        presetKey: 'arrow-up',
        options: {
    ...
    await test.step('Проверить что смена фигуры использует исходный replace box, а не выросший размер', () => {
      shapes.checkUpdate({
        shape: updatedShape,
        presetKey: SHAPE_PRESERVE_ASPECT_REPLACEMENT_PRESET
      })
