...
test.describe('Смена фигуры после roundtrip через разные менеджеры', () => {
  test('после сохранения фигуры в шаблон следующая замена использует исходный replace box', async({
    canvas,
    editorModel,
    shapes,
    template
  }) => {
    const sourceShape = await test.step('Подготовить фигуру, которая выросла под текст после замены', async() => {
    ...
    const restoredShape = await test.step('Получить состояние фигуры после шаблонного roundtrip', () => {
    ...
    const finalShape = await test.step('Получить состояние после следующей замены', () => {
      return shapes.getObject({ objectIndex: 0 })
    })
    ...
    await test.step('Проверить что шаблон сохранил replace box для следующей замены', () => {
      ...
      expect(Math.abs(restoredSnapshot.groupBoundsHeight - sourceShape.groupBoundsHeight))
        .toBeLessThanOrEqual(SHAPE_REPLACE_TOLERANCE)
      expect(restoredShape?.shapeReplaceBoxWidth).toBe(expectedReplaceBoxWidth)
      expect(restoredShape?.shapeReplaceBoxHeight).toBe(expectedReplaceBoxHeight)
  ...
  test('после копирования и вставки следующая замена использует исходный replace box', async({
    clipboard,
    editorModel,
    shapes
  }) => {
    ...
    const restoredShape = await test.step('Получить состояние вставленной копии', () => {
    ...
    const finalShape = await test.step('Получить состояние после замены вставленной копии', () => {
      return shapes.getObject({ id: pastedShapeId })
    })
    ...
    await test.step('Проверить что clipboard roundtrip сохранил replace box для следующей замены', () => {
      const expectedReplaceBoxWidth = SHAPE_REPLACE_BASE_OPTIONS.width ?? 0
      const expectedReplaceBoxHeight = SHAPE_REPLACE_BASE_OPTIONS.height ?? 0
  ...
  test('после undo и redo следующая замена использует исходный replace box', async({
    editorModel,
    history,
    shapes
  }) => {
    ...
    const restoredShape = await test.step('Получить состояние восстановленной фигуры', () => {
      return shapes.getObject({ id: 'shape-replace-history-source' })
    })
    ...
    await test.step('Проверить что history roundtrip сохранил replace box для следующей замены', () => {
      const expectedReplaceBoxWidth = SHAPE_REPLACE_BASE_OPTIONS.width ?? 0
      const expectedReplaceBoxHeight = SHAPE_REPLACE_BASE_OPTIONS.height ?? 0
