...
test('при микродвижениях общее выделение сохраняет равноудалённость по горизонтали', async({
  activeSelectionHorizontalSpacingSetup: setup,
  snapping
}) => {
  const freeTop = setup.left.boundsTop + 40
...
test('при микродвижениях общее выделение сохраняет равноудалённость по вертикали', async({
  activeSelectionVerticalSpacingSetup: setup,
  snapping
}) => {
