...
test.beforeEach(async({
  ...
  baselineSelection = await selection.scaling.getSnapshot()
...
test('во время скейлинга увеличивает выделение и оба шейпа без сдвига левого верхнего угла', async({
  ...
  const started = await selection.scaling.startFromControl({ control: 'br' })
  const liveSelection = await selection.scaling.dragControlBy({
    deltaX: 30,
    deltaY: 20
  })
