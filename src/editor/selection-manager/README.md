...
- [`scaling/active-selection-scale-composition.ts`](./scaling/active-selection-scale-composition.ts) identifies supported image, shape, and text-driven compositions, captures the child state protected during a gesture, and verifies that scaling preserved its invariants.
...
The interaction controller and the composition contract are separated so adding another supported composition does not mix eligibility and invariant checks with browser interaction lifecycle. The next migration step is the full composition of an image, a shape, and standalone text. Individually rotated text children, noncanonical images in text-driven compositions, `Group`, `CropFrame`, nested objects, and unknown child types remain outside the supported boundary until their product contract is established separately.
