...
Nested, flipped, skewed, axis-locked, or non-canonical shapes and unsupported Fabric transforms are deliberately routed to the legacy scaling path before the new owner performs a mutation. Direct canonical shapes with their own rotation also remain outside unified snapping, but their legacy `ActiveSelection` preview uses an immutable local centre and angle. Non-uniform selection scale changes their canonical width and/or height without introducing visible skew, and the last live child geometry and selection frame are preserved when Fabric rebuilds the selection on `object:modified`. This correction applies to shape-only and mixed selections whose temporary frame has no flip or skew; it does not migrate those selections to the unified snapping owner. Shape-only selections of unrotated canonical shapes support all eight side and corner controls, rotation of the entire selection, centred scaling, proportional and free corner scaling, independent guide holding, Ctrl, zoom, pan, and the existing minimum-size and text-layout rules. A side-control skew stays on the existing Fabric path, clears unified guides, and leaves the transform on the temporary selection. Exact materialization and history or template round trips for skew require a separate affine contract. Selections containing text, images mixed with shapes, `Group`, `CropFrame`, or unknown objects also remain on the legacy snapping path.
