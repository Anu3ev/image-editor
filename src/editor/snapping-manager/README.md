...
## Image-only, shape-only, and text-driven ActiveSelection scale integration

`SelectionManager` owns the migrated scale session when the active target is an `ActiveSelection` containing at least two direct `FabricImage` children, at least two direct canonical shape groups, or at least one supported standalone `BackgroundTextbox` and two objects in total. A text-driven composition may contain supported text objects and canonical direct images. The selection must use positive scale, supported side or corner controls, and no skew, flip, parent group, lock, or locked scale axis; a text-driven selection must also start with unit scale. Shape children additionally require unit scale and zero angle, skew, and flip. Text children require unit scale and zero angle. They must also have no path or parent, no skew or flip, zero stroke width, no editing or locked state, and no locked scale axis. Images in a text-driven composition may retain positive finite scale but require zero angle, skew, flip, and stroke width, no parent, and no locked scale axis. Unknown, nested, individually rotated, shape-and-text, `Group`, and `CropFrame` compositions deliberately remain on the existing snapping path.

The controller captures the exact selection bounds, fixed scene point, original selection scale, candidates, zoom, and the protected child state on `mousedown`. Fabric may apply its normal preview first; the controller then resolves the pointer step from the immutable baseline, applies one final `scaleX/scaleY` pair to the selection, synchronizes the active transform, restores the fixed point, and verifies the final exact bounds before publishing guides. For shapes, the early `ShapeManager` listener asks this owner to process the step immediately and uses the existing path only if the step is declined. The controller then asks `ShapeManager` to intersect child constraints and recalculate shape and text layout once before verification. A duplicate Fabric event and `mouse:move` marker cannot apply the result twice.

Image-only and shape-only selections support all eight controls. Text-driven selections support the two horizontal side controls and four corner controls; the vertical side controls stay hidden. Side, proportional corner, Shift-controlled free corner, centred, and whole-selection rotated gestures preserve their fixed point. Hold and release are resolved independently by axis, and zoom or pan affects only the scene-to-viewport conversion. The session is cleared on `mouseup`, selection changes, removal of the selection or one of the children captured at gesture start, `pointercancel`, `touchcancel`, window blur, and manager destruction. Pointer cancellation, window blur, and editor-owned deletion finish the current Fabric transform before transient state is cleared. If the user presses Shift on a text side control after a confirmed scale step, the last canonical text geometry and calculated image geometry are committed, the current transform ends without applying skew, and a later gesture starts with fresh snapping state.

During an image-only gesture, scale belongs to the temporary `ActiveSelection`; local image position, dimensions, scale, crop, angle, skew, flip, and origins remain unchanged. During a supported shape-only gesture, the selected shapes keep unit scale while their canonical dimensions, placement, and text layout follow the accepted selection scale. During a text-driven gesture, `TextManager` measures wrapping and canonical text properties before changing the live objects, projects direct images from the same local baseline, applies one result, and keeps text scale inverse to the temporary selection scale. On `object:modified`, the measured text properties and calculated image geometry are committed, and the restored selection returns to unit scale without changing the last visible frame. Image source dimensions, crop, angle, skew, flip, and origins remain unchanged. Exact text geometry is restored through history. Copying and template restoration are verified for text-only selections and remain open for text-driven selections that also contain images. Direct shape or text children with their own rotation and noncanonical image children still use legacy snapping.

Do not extend this matcher to selections containing shapes and text without adding the same explicit routing boundary first. Their domain mutation and commit paths must be coordinated before one pointer step can be owned safely.
...
- Image scale is complete for the supported top-level `FabricImage` boundary. Scaling `ActiveSelection` is complete for image-only, shape-only, and text-only compositions; the current working slice also routes a composition of canonical images and standalone text through one owner. Movement is complete for top-level images, shapes, standalone `Textbox` objects, eligible `ActiveSelection` compositions, and direct top-level `Group` objects. Side-width resizing and corner scaling are complete for the supported standalone text boundary. The next `ActiveSelection` scale stage is the full image, shape, and text composition.
