...
- [`scaling/rectangular-scale-interaction.ts`](./scaling/rectangular-scale-interaction.ts) selects the current Fabric scale mode, resolves preview or pointer multipliers, applies one scale plan around the fixed scene point, and reads the exact final geometry. The single-image and image-only selection owners share these operations while keeping their eligibility and protected-state rules separate.
- [`scaling/standard-scale-control.ts`](./scaling/standard-scale-control.ts) verifies that a Fabric control keeps the standard rectangular scale action and detects a side control switching to skew. Image and image-only selection owners share this compatibility boundary.
...
- [`../selection-manager/scaling/active-selection-scale-interaction-controller.ts`](../selection-manager/scaling/active-selection-scale-interaction-controller.ts) owns supported scale sessions for an `ActiveSelection` made only of direct top-level images. It applies one transform to the temporary selection and keeps every child's local geometry unchanged.
...
The new path supports side and corner controls, rotation, centred scaling, free corner scaling with Shift, and disabling snapping with Ctrl. Skewed, flipped, nested, or axis-locked shapes, as well as shape-containing `ActiveSelection`, use the legacy path. A supported gesture that later crosses the fixed point may hand the remaining live interaction back to the legacy path after earlier unified steps have already been applied.
...
## Image-only ActiveSelection scale integration

`SelectionManager` owns the migrated scale session when the active target is an `ActiveSelection` containing at least two direct top-level `FabricImage` children. The selection must use positive scale, standard side or corner controls, and no skew, flip, parent group, lock, or locked scale axis. Unknown children, nested images, shape or text children, `Group`, and `CropFrame` deliberately remain on the existing path.

The controller captures the exact selection bounds, fixed scene point, original selection scale, candidates, zoom, and every child's protected local geometry on `mousedown`. Fabric may apply its normal preview first; the controller then resolves the pointer step from the immutable baseline, applies one final `scaleX/scaleY` pair to the selection, synchronizes the active transform, restores the fixed point, and verifies the final exact bounds before publishing guides. A duplicate Fabric event and `mouse:move` marker cannot apply the result twice.

All eight standard controls use the same owner. Side, proportional corner, Shift-controlled free corner, centred, and rotated gestures preserve their fixed point. Hold, release, and Ctrl are resolved independently by axis, and zoom or pan affects only the scene-to-viewport conversion. The session is cleared on `mouseup`, selection changes, removal of the selection or one of the images captured at gesture start, `pointercancel`, `touchcancel`, window blur, and manager destruction. Pointer cancellation and window blur end the current Fabric transform before clearing snapping state and guides.

During the live gesture, scale belongs to the temporary `ActiveSelection`; local image position, dimensions, scale, crop, angle, skew, flip, and origins remain unchanged. After history restoration the temporary selection no longer exists, so the same visible geometry may be represented by scale on the restored top-level images. Tests therefore compare restored visible bounds and self-consistent image geometry rather than requiring the transient live representation to survive serialization.

Do not extend this matcher to shape, standalone-text, or mixed selections. Those compositions currently have earlier ShapeManager or TextManager mutation and commit paths. They require an explicit composite routing boundary and domain-specific child materialization before they can share this owner without applying the same pointer step more than once.
...
- Image scale is complete for the supported top-level `FabricImage` boundary and for image-only `ActiveSelection`. Movement is complete for top-level images, shapes, standalone `Textbox` objects, eligible `ActiveSelection` compositions, and direct top-level `Group` objects. Side-width resizing and corner scaling are complete for the supported standalone text boundary. The next `ActiveSelection` scale stages are shape-only, text-only, and mixed compositions with one explicit composite owner.
