...
For a standalone top-level image or top-level shape, `mousedown` captures exact initial bounds, Fabric `left/top`, zoom, named line candidates, and spacing candidates. Every `object:moving` step reads the raw Fabric result, resolves line and equal-spacing hold state independently by axis, includes pixel rounding in the same final position, applies at most one post-Fabric translation, and verifies the resulting exact bounds before publishing guides. Equal spacing is applied from the exact selected neighbours or reference pattern; neither centred nor reference spacing quantizes the correction to a half-pixel step, and display rounding does not determine the final object position. Related intervals are kept only when they lead to the same exact position, with a small tolerance for floating-point noise.
...
Shape movement remains a plain Fabric translation: it does not run shape layout, change canonical dimensions, or alter the embedded text. The regular Fabric history and `object:modified` lifecycle still commit the completed drag.
