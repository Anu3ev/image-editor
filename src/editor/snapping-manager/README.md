...
The two-phase contract is currently used when scaling a regular top-level shape, when moving a standalone top-level `FabricImage`, top-level shape, standalone top-level `Textbox`, or an eligible `ActiveSelection`, and when scaling a supported top-level `FabricImage` through any of the eight standard Fabric controls. Movement of `Group`, `CropFrame`, nested objects, and ineligible `ActiveSelection` compositions still uses the legacy path. Unsupported image scale geometries and controls with custom scale behaviour or geometry are not routed through the migrated scale owner. The exact migration boundary and the recommended order for continuing the work are documented in [`technical-specifications/unified-scale-snapping-current-state.md`](../../../technical-specifications/unified-scale-snapping-current-state.md).
...
- [`movement/movement-snapping-controller.ts`](./movement/movement-snapping-controller.ts) captures an immutable movement session and applies one final Fabric translation for standalone top-level images, top-level shapes, standalone top-level `Textbox` objects, and eligible `ActiveSelection` compositions. Other object types are deliberately routed to the legacy movement path.
...
An `ActiveSelection` containing `Group`, `CropFrame`, an object selected out of a parent group, an unknown type, or non-canonical text scaling is deliberately left on the legacy path. Direct `Group` movement also remains there until its persisted hierarchy, child coordinate plane, serialization, and history lifecycle are audited separately.

This phase intentionally handles only an active browser drag of a top-level `FabricImage`, top-level shape, standalone top-level `Textbox`, or eligible `ActiveSelection`. Movement without an active browser gesture, nested objects, `Group`, `CropFrame`, and ineligible `ActiveSelection` compositions continue through the legacy path.
...
- Image scale is complete for the supported top-level `FabricImage` boundary, and movement is complete for top-level images, shapes, standalone `Textbox` objects, and eligible `ActiveSelection` compositions. The next movement slice is `Group`, after a separate capability audit of its persisted hierarchy and child coordinate plane.
