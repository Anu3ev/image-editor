...
The two-phase contract is currently used when scaling a regular top-level shape, when moving a standalone top-level `FabricImage`, and when scaling a supported top-level `FabricImage` through any of the eight standard Fabric controls. Movement of shapes, standalone text, `ActiveSelection`, `Group`, and `CropFrame` still uses the legacy path. Unsupported image scale geometries and controls with custom scale behaviour or geometry also remain on that path. The exact migration boundary and the recommended order for continuing the work are documented in [`technical-specifications/unified-scale-snapping-current-state.md`](../../../technical-specifications/unified-scale-snapping-current-state.md).
...
- [`movement/image-movement-snapping-controller.ts`](./movement/image-movement-snapping-controller.ts) captures an immutable movement session and applies one final translation for standalone top-level images. Other object types are deliberately routed to the existing owner.
...
This phase intentionally supports only a top-level `FabricImage`. Image movement without an active browser gesture and movement of shapes, standalone text, `ActiveSelection`, `Group`, and `CropFrame` continue through the legacy path.
...
- Image scale is complete for the supported top-level `FabricImage` boundary. The next migration slice is another concrete object type or movement owner, not another special-case Image control.
