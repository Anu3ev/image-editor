...
- [`../selection-manager/scaling/active-selection-scale-interaction-controller.ts`](../selection-manager/scaling/active-selection-scale-interaction-controller.ts) owns supported scale sessions for an `ActiveSelection` made only of direct top-level images or only of canonical direct top-level shapes. It applies one transform to the temporary selection and delegates shape constraints and layout without starting a second snapping session.
...
## Image-only and shape-only ActiveSelection scale integration

`SelectionManager` owns the migrated scale session when the active target is an `ActiveSelection` containing at least two direct top-level `FabricImage` children or at least two direct canonical shape groups. The selection must use positive scale, supported side or corner controls, and no skew, flip, parent group, lock, or locked scale axis. Shape children additionally require unit scale and zero angle, skew, and flip. A shape-only selection may use the four corner controls installed by `ShapeManager`; their explicit marker identifies the same rectangular geometry and supplies the proportional or free mode used by the control itself. Unknown, nested, individually rotated, mixed, standalone-text, `Group`, and `CropFrame` compositions deliberately remain on the existing snapping path.
...
All eight controls use the same owner while they perform rectangular scaling. Side, proportional corner, Shift-controlled free corner, centred, and whole-selection rotated gestures preserve their fixed point. Hold, release, and Ctrl are resolved independently by axis, and zoom or pan affects only the scene-to-viewport conversion. The session is cleared on `mouseup`, selection changes, removal of the selection or one of the children captured at gesture start, `pointercancel`, `touchcancel`, window blur, and manager destruction. Pointer cancellation, window blur, manager destruction, and editor-owned deletion finish the current Fabric transform before transient shape state is cleared. If a side control switches to skew, unified snapping stops and clears its guides. The current Fabric selection remains responsible for skew frames; releasing Shift hands subsequent scaling frames to the existing shape path.

During an image-only gesture, scale belongs to the temporary `ActiveSelection`; local image position, dimensions, scale, crop, angle, skew, flip, and origins remain unchanged. During a supported shape-only gesture, the selected shapes keep unit scale while their canonical dimensions, placement, and text layout follow the one accepted selection scale. A completed rectangular shape gesture is materialized into canonical child geometry on `object:modified`. Direct canonical shape children with their own rotation still use legacy snapping, but `ShapeManager` compensates the parent scale so their visible angle and zero-skew geometry remain stable during side and corner scaling and after `mouseup`. This correction also covers mixed selections without moving guide ownership into `SelectionManager`. A side-control skew is not materialized as a rectangular resize and remains on the temporary `ActiveSelection`. Exact child materialization and history or template round trips for this affine case are outside the current migration boundary. After history restoration the temporary selection no longer exists, so tests compare visible bounds and each domain object's canonical geometry rather than requiring the transient selection representation to survive serialization.

Do not extend this matcher to standalone-text or mixed selections without adding the same explicit routing boundary first. Those compositions still have earlier domain mutation and commit paths and would otherwise apply one pointer step more than once.
...
- Image scale is complete for the supported top-level `FabricImage` boundary. Scaling `ActiveSelection` is complete for image-only and shape-only compositions. Movement is complete for top-level images, shapes, standalone `Textbox` objects, eligible `ActiveSelection` compositions, and direct top-level `Group` objects. Side-width resizing and corner scaling are complete for the supported standalone text boundary. The next `ActiveSelection` scale stages are text-only and mixed compositions with one explicit composite owner.
