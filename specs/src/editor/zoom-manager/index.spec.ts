...
describe('ZoomManager', () => {
  ...
  describe('focus aware centering', () => {
    it('должен запоминать точку фокуса при zoom-in с координатами', () => {
      mockCanvas.getZoom.mockReturnValue(1)
      zoomManager.zoom(0.2, { pointX: 300, pointY: 200 })

      const lastFocus = (zoomManager as any).lastZoomFocusPoint
    ...
    it('должен очищать точку фокуса при достижении defaultZoom', () => {
      (zoomManager as any).lastZoomFocusPoint = new Point(250, 150)
      mockCanvas.getZoom.mockReturnValue(0.9)

      zoomManager.zoom(-0.2)

      expect((zoomManager as any).lastZoomFocusPoint).toBeNull()
    })
    ...
    it('ограничивает шаг центрирования, чтобы точка фокуса оставалась во вьюпорте', () => {
      ...
      const step = (zoomManager as any)._constrainStepToKeepPointVisible(
        { x: 500, y: -500 },
        focusPoint,
        zoom,
        viewportTransform
